...
class Autocomplete extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    const options = name === 'options' ? newValue.split(',') : []
...
function autocomplete(el, arr) {
  ...
  const inp = shadow.querySelector('#myInput')
  ...
  inp.addEventListener("input", function(e) {
    ...
    shadow.querySelector('.autocomplete').appendChild(a);
    console.log(a)
  ...
  inp.addEventListener("keydown", function(e) {
    var x = shadow.getElementById(this.id + "autocomplete-list");
  ...
  function closeAllLists(elmnt) {
    /*close all autocomplete lists in the shadow,
    except the one passed as an argument:*/
    var x = shadow.querySelectorAll(".autocomplete-items");
    for (var i = 0; i < x.length; i++) {
